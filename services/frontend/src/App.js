...
function App() {
  ...
  // Persist the cart so a refresh keeps the items the user added (cleared only on
  // explicit remove or after an order is placed).
  useEffect(() => {
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      setSuccess(
        `Order placed! ID: ${order.id} — Total: $${order.total.toFixed(2)}`,
      );
      setCart([]);
      setPage("orders");
      fetchOrders();
      ...
      <header style={styles.header}>
        ...
        <nav style={styles.nav}>
          ...
          {user ? (
            ...
            <button style={styles.navBtn} onClick={() => setShowAuth(true)}>
              Login
            </button>
          )}
        </nav>
  ...
  return (
    <>
      ...
      <table style={styles.table}>
        ...
        <tfoot>
          <tr>
            <td colSpan={3} style={{ ...styles.td, fontWeight: "bold" }}>
              Total
            </td>
            <td
              style={{
